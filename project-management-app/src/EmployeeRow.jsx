...
export default function EmployeeRow({ employee, tasks, currentRole, moveTask, deleteTask, addComment }) {
  return (
      ...
      <div style={styles.tasksSection}>
        {tasks.length === 0 ? (
          ...
          <div style={styles.tasksGrid}>
            {tasks.map(task => (
              <TaskItem key={task.id} task={task} currentRole={currentRole} moveTask={moveTask} deleteTask={deleteTask} addComment={addComment} />
            ))}
          </div>
        )}
      </div>
...
const styles = {
  rowCard: { display: 'flex', backgroundColor: '#111827', borderRadius: '12px', border: '1px solid #1f2937', overflow: 'hidden', minHeight: '110px' },
  ...
  idBadge: { fontFamily: 'monospace', fontSize: '11px', color: '#6366f1', backgroundColor: 'rgba(99, 102, 241, 0.1)', padding: '2px 6px', borderRadius: '4px', display: 'inline-block', marginTop: '4px' },
  ...
  tasksGrid: { display: 'flex', flexWrap: 'wrap', gap: '12px', width: '100%' },
  noTasks: { color: '#4b5563', fontSize: '14px', fontStyle: 'italic', paddingLeft: '10px' }
};
